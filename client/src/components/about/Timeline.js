      ...
      <div className="relative max-w-3xl mx-auto">
        ...
        {timelineData.map((item, index) => (
          ...
          >
            ...
            <div className="md:hidden w-1/12"></div>
            <div className="md:hidden z-20 flex items-center justify-center bg-blue-500 shadow-xl w-12 h-12 rounded-full">
              <span className="text-white font-bold text-lg">{item.year}</span>
            </div>
            <div className="md:hidden bg-white dark:bg-gray-800 rounded-lg shadow-xl w-9/12 px-4 py-3 transform hover:scale-105 transition-transform duration-300 transition-colors duration-800 ease-in-out text-left">
              <h3 className="font-sans font-bold text-xl mb-1">{item.title}</h3>
              <h4 className="font-sans font-semibold text-md text-blue-500 mb-2">{item.company}</h4>
              <p className="font-sans text-sm text-gray-600 dark:text-gray-300 leading-relaxed transition-colors duration-800 ease-in-out">{item.description}</p>
            </div>

            {/* --- Desktop Layout --- */}
            {/* On desktop, a spacer pushes content to either side */}
            <div className="hidden md:block w-5/12"></div>
            <div className="hidden md:flex z-20 items-center justify-center bg-blue-500 shadow-xl w-16 h-16 rounded-full">
